...
def parse_GALRawObsUbx(block):
    tow = int(struct.unpack("d", block[:8])[0])
...
def myUbloxPages2GalileoICD(dwrds):
    # print(dwrds)
    # print(correctEndianess(dwrds))
    # dwrds = list(struct.unpack("8I", dwrds))
    page_string = "{:032b}," * 8
    data_page_bits = page_string.format(*dwrds)
    # Format as list and cut extra element
    data_page_bits = data_page_bits.split(",")[0:8]
...
class UBX:
    ...
    def __next__(self):
        ...
        while sync1 := self.file.read(1):
            if sync1 == SYNC_HEADER1:
                sync2 = self.file.read(1)
                if sync2 == SYNC_HEADER2:
                    header = sync1 + sync2
                    for _ in range(0, 4):
                        header = header + self.file.read(1)

                    messageType, length = parse_header(header)
                    if length == 0:
                    ...
                    # Read every byte in the message from socket
                    fullMessage = bytearray(header)
                    while len(fullMessage) < length + 8:
                        fullMessage.extend(self.file.read(1))

                    payload = fullMessage[6:-2]

                    if len(fullMessage) != length + 8:
                        print(f"Missing bytes: {length+8 - len(fullMessage)}")
                        self.file_pos += 1
                        self.file.seek(self.file_pos)
                        continue

                    if not isChecksumOk(fullMessage):
                        print(f"Checksum ERROR in message {messageType}")
                        self.file_pos += 1
                        self.file.seek(self.file_pos)
                        continue

                    parsedMessage = UBXReader.parse(bytes(fullMessage))

                    if parsedMessage.identity == SFRBX:
                        gnssId = parsedMessage.gnssId
                        svId = parsedMessage.svId
                        reserved1 = parsedMessage.reserved0
                        numWords = parsedMessage.numWords
                    
                    elif parsedMessage.identity == RAWX:

                        a, b, leapS = parse_GALRawObsUbx(payload)
                        c = math.floor(parsedMessage.rcvTow)
                        d = parsedMessage.week
                        print(f"OBS {a} {b} {c} {d}")

                        continue
                    
                    elif parsedMessage.identity == TIMEGAL:
                        self.wn_c = parsedMessage.galWno+1024
                        self.tow = parsedMessage.iTOW // 1000
                        # print(self.tow, parsedMessage.iTOW)
                        print(f"GST  NOW {self.wn_c} TOW {self.tow}")
                        continue                
                    else:
                        continue

                    if gnssId != GAL_GNSS_ID:
                        continue
                    if svId > MAX_GAL_SATS:
                        raise ValueError("Unexpected Galileo satellite detected.")

                    # Keep E1 signals only
                    if (reserved1 != 1):
                        continue

                    # Save datawords into variable
                    dataWords = [parsedMessage.dwrd_01,
                                parsedMessage.dwrd_02,
                                parsedMessage.dwrd_03,
                                parsedMessage.dwrd_04,
                                parsedMessage.dwrd_05,
                                parsedMessage.dwrd_06,
                                parsedMessage.dwrd_07,
                                parsedMessage.dwrd_08]

                    nav_msg_even, nav_msg_odd, nav_msg_original = \
                        myUbloxPages2GalileoICD(dataWords)
                    ...
                    # print(f"SatId= {svId+32} Computed CRC: {computed_crc} Read CRC: {int(read_crc, 16)}")

                    # nav_bits = BitArray(bin="".join([nav_msg_even,
                    #                                 nav_msg_odd]))[:234]
                    nav_bits = BitArray(bin="".join([nav_msg_even,
                                        nav_msg_odd]))
                    wn_c = self.wn_c - 1024 if self.wn_c > 1024 else 0
                    tow = self.tow - 2

                    # nav_bits.insert("0b000000", 114)
                    data_format = DataFormat(
                        svId, wn_c, tow, nav_bits, "GAL_L1BC", isCRCOK
                    )
                    break
...
class UBXLive:
    ...
    def __next__(self):
        ...
        self.index += 1
        try:
            while sync1 := self.s.recv(1):
                if sync1 == SYNC_HEADER1:
                    sync2 = self.s.recv(1)
                    if sync2 == SYNC_HEADER2:
                        header = sync1 + sync2
                        for _ in range(0, 4):
                            header = header + self.s.recv(1)
                        ...
                        fullMessage = bytearray(header)
                        while len(fullMessage) < length + 8:
                            fullMessage.extend(self.s.recv(1))
                        ...
                        if parsedMessage.identity == SFRBX:
                            ...
                            # print(f"GNSS {gnssId} svId {svId} frequency {reserved1}")
                        # elif parsedMessage.identity == RAWX:
                        #     print(f"RAWX NOW {parsedMessage.week} TOW {round(parsedMessage.rcvTow)} frequency {reserved1}")
                            
                        #     continue
                        elif parsedMessage.identity == TIMEGAL:
                            self.wn_c = parsedMessage.galWno+1024
                            self.tow = parsedMessage.iTOW
                            # print(f"GST  NOW {self.wn_c} TOW {self.tow}")
                            continue
                        else:
                            continue
                        ...
                        computed_crc = computeCRC(nav_msg_original)
                        # Read CRC
                        read_crc = int(hex(int(nav_msg_original[128+82:128+82+24], 2)), 16)

                        if computed_crc != read_crc:
                            continue

                        if (self.tow == 0 and self.wn_c == 0):
                            continue
                        ...
                        data_format = DataFormat(
                            svId, wn, tow, nav_bits, "GAL_L1BC", True
                        )
                        break
